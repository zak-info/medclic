...
const Paniers = ({ data,user }) => {
    ...
    const handleFilter = (text)=>{
        setExchanges(data?.filter(item => item?.idUser?.fullname?.toLowerCase()?.includes(text.toLowerCase())))
    }
    return (
        <div className='w-screen h-screen pt-8 flex flex-col items-center'>
            <div className='w-full flex justify-between px-8'>
                <h1 className='text-xl font-bold w-2/3'>Panier</h1>
             ...
             {/* <div className='w-full px-10  flex items-center mt-6 gap-4'>
                <Input type="text" label={"rechercher par le nom de client"} onChange={(e) => {handleFilter(e.target.value)}} className='w-full ' required />
            </div> */}
            {
                exchanges?.map((ph, index) => (
                    <Card key={index} data={{...ph,imageUrl:'/images/dwa.svg'}} />
                ))
            }
        </div>
    )
