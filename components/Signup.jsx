...
const Signup = ({ type }) => {
    ...
    const Login = async (e) => {
        ...
        setPostloader(true);
        try {
            const result = await createUser({ type, fullname: e.target.fullname.value,phone: e.target.phone.value,address: e.target.address.value, email: e.target.email.value, password: e.target.password.value })
            console.log(result);
            router.push("/login")
        } catch (error) {
            console.log(error);
        ...
        <div className='w-screen h-screen flex justify-center items-center'>
            <form onSubmit={Login} className='relative w-full h-full  md:w-1/4 md:h-5/6 border rounded-xl flex flex-col items-center pt-8'>
                ...
                <div className='w-4/5 h-12 mt-12 border rounded-2xl border-primary-1-hover flex items-center '>
                    ...
                    <input type="text" name='fullname' className='ms-2 w-2/3 outline-none border-none bg-none text-sm' placeholder='enter your fullname' />
                </div>
                <div className='w-4/5 h-12 mt-4 border rounded-2xl border-primary-1-hover flex items-center '>
                    ...
                    <input type="text" name='phone' className='ms-2 w-2/3 outline-none border-none bg-none text-sm' placeholder='enter your phone' />
                </div>
                <div className='w-4/5 h-12 mt-4 border rounded-2xl border-primary-1-hover flex items-center '>
                    <i class="ri-map-pin-line text-2xl ms-4 text-primary-1"></i>
                    <input type="text" name='address' className='ms-2 w-2/3 outline-none border-none bg-none text-sm' placeholder='enter your address' />
                </div>
                <div className='w-4/5 h-12 mt-4 border rounded-2xl border-primary-1-hover flex items-center '>
                    ...
                    <input type="email" name='email' className='ms-2 w-2/3 outline-none border-none bg-none text-sm' placeholder='enter your email' />
                </div>
                ...
                <div className='w-4/5 h-12 mt-4 border rounded-2xl border-primary-1-hover flex items-center '>
                    ...
                    <input type={eyeOff ? "" : "password"} name='password' className='ms-2 w-2/3 outline-none border-none bg-none text-sm' placeholder='enter password' />
                    {
                        !eyeOff ?
